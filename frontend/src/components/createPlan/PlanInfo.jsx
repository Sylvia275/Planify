import React, { useState } from "react";
import Stage from "./Stage";

// Import the CSS
import "../../styles/PlanInfo.css";

const CATEGORIES = [
  "Study",
  "Work",
  "Personal",
  "Health",
  "Fitness",
  "Language",
  "Exam",
  "Project"
];

const PlanInfo = () => {
  const [stages, setStages] = useState([0]);
  const [showCategories, setShowCategories] = useState(false);
  const [selectedCategories, setSelectedCategories] = useState([]);

  const addStage = () => {
    setStages([...stages, stages.length]);
  };

  const toggleCategory = (category) => {
    setSelectedCategories((prev) =>
      prev.includes(category)
        ? prev.filter((c) => c !== category)
        : [...prev, category]
    );
  };

  return (
    <div className="planinfo-wrapper">
      {/* Plan Title */}
      <div className="plan-title-card">
        <label htmlFor="plan-title">Title</label>
        <input
          id="plan-title"
          type="text"
          placeholder="Enter plan title"
          className="plan-title-input"
        />
      </div>

      {/* Plan Info Card */}
      <div className="planinfo-card">
        {/* Image Upload */}
        <div className="image-upload" role="button" tabIndex={0}>
          <input type="file" accept="image/*" aria-label="Upload plan image" />
          <span>Click to upload<br />plan image</span>
        </div>

        {/* Description + Categories */}
        <div className="planinfo-right">
          <div className="planinfo-field">
            <label htmlFor="plan-description">Description</label>
            <textarea
              id="plan-description"
              placeholder="Describe your plan..."
            />
          </div>

          <div className="categories-section">
            <button
              className="categories-btn"
              onClick={() => setShowCategories(!showCategories)}
              aria-expanded={showCategories}
              aria-controls="categories-popup"
            >
              Categories {selectedCategories.length > 0 && `(${selectedCategories.length})`}
            </button>

            {showCategories && (
              <div className="categories-popup" id="categories-popup">
                {CATEGORIES.map((cat) => (
                  <span
                    key={cat}
                    className={`category-tag ${selectedCategories.includes(cat) ? "active" : ""}`}
                    onClick={() => toggleCategory(cat)}
                    role="button"
                    tabIndex={0}
                    onKeyDown={(e) => e.key === "Enter" && toggleCategory(cat)}
                  >
                    {cat}
                  </span>
                ))}
              </div>
            )}
          </div>
        </div>
      </div>

      {/* Stages */}
      <div className="stage-list">
        {stages.map((_, index) => (
          <Stage key={index} stageNumber={index + 1} />
        ))}

        <button className="add-stage-btn" onClick={addStage}>
          + Add Stage
        </button>
      </div>
    </div>
  );
};

export default PlanInfo;